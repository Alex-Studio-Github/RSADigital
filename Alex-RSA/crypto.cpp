...
int packbits(unsigned char * buffin,int buffin_len, unsigned char * buffout, unsigned char * fileout,unsigned char *host,int base64){
	...
	*len is the size of plaintext in bytes */
	...
	len = c_len + f_len;
	/* Cifrado asimetrico********************/
	if(directoryexist((char*)BASE_DIR)==0)
		keygen();
        // Aqui se debe indicar con que llave publica deseamos cifrar	
	
	if(host!=NULL)
		pukey  = getpublicRSAkey((char*)host);
	else
		pukey  = getpublicRSAkey((char*)"localhost");
	char *encryptrsa =(char *) malloc(RSA_size(pukey));
	int   encryptrsa_len;

	if((encryptrsa_len = RSA_public_encrypt((AES_KEY_SIZE/8)+1, (unsigned char*)aes_key,(unsigned char*)encryptrsa,
	    pukey, RSA_PKCS1_OAEP_PADDING)) == -1)
		;//((char*)"Bad RSA encryption");
	...
	{	
		writectofile(fileout,(unsigned char*)encryptrsa,encryptrsa_len,iv,ciphertext,len);	
	}
	/* Fin salidas *********************/
...
int unpackbits(unsigned char *buffin,unsigned char *buffout, unsigned  char *fname,unsigned char *fileout,int base64){
	...
	if(fname!=NULL){
		...
		if(base64==1){
			...
			readmsgfromfile(&b64str,(char*)fname);
			//printf("Decoding from Base64(%d)...\n%s\n",n,b64str);
			n=dec_base64((char*)b64str, &buffin);
			//hex_print(buffin,n);
		}
		else
			n=readcipherfile(&buffin,(char*)fname);
		...
		;//((char*)"Option -in not yet implemented for -d");
	/***********************************************************************/
	
	/** Descifrado asimetrico **********************************************/
	int decrypt_len=AES_KEY_SIZE/8;
	RSA *prikey=getprivateRSAkey();
	if(RSA_private_decrypt(256, (unsigned char*)ckey, (unsigned char*)aes_key, prikey, RSA_PKCS1_OAEP_PADDING) == -1)
 		;//((char*)"Bad RSA decryption");
	/***********************************************************************/
...
int keygen(void){
	...
	if(directoryexist((char*)BASE_DIR)==0)
		createdirestructure();
	//error_t
	if(0 != (fopen_s(&privf , PRIV_KEY_PATH, "w"))){
	 	PEM_write_RSAPrivateKey(privf, keypair,NULL,NULL, 0,NULL,NULL);
	}
	else
		;//((char*)"Generacion de llave privada private.key");
	//FILE* pubf;
	pubf = NULL;
	if(0 != fopen_s(&pubf,PUB_KEY_PATH,"w")){
		 PEM_write_RSAPublicKey(pubf,keypair);
	}
	else
		;//((char*)"Generacion de llave publica public.key");
	fclose(privf);
	fclose(pubf);
	addtowellknownhosts((char*)"localhost",(char*)PUB_KEY_PATH);
	printf("\n%s\n%s\n", pri_key, pub_key);
	return 1;
}
int writetofileb(unsigned char *fileout,unsigned char *m,int size){
	FILE *file;
	fopen_s(&file,(const char*)fileout,"w");
	if(!file)
		;//((char*)"El archivo de salida no se ha podido crear");
...
int writectofile(unsigned char *fileout,unsigned char *key,int keylen,unsigned char *iv,unsigned char *text,int sizec){
	FILE *file;
	fopen_s(&file,(const char*)fileout,"wb");
	int i;
	if(!file)
		;//((char*)"El archivo de salida no se ha podido crear");
	struct recordsb *my_record= (recordsb *)malloc(sizeof(struct recordsb));
	/*************************************************************************/
...
int sizeofcipherfile(unsigned char *name){
	...
	fopen_s(&file,(const char*)name ,"rb");
	if(!file)
		;//((char*)"El archivo no se ha podido abrir");
...
int readcipherfile(unsigned char** buffin,char *name)
{
	...
	//Allocate memory
	*buffin =(unsigned char *)malloc(fileLen+1);
	if (!*buffin){
...
int readmsgfromfile(unsigned char **buffin,char *name){
	...
	fopen_s(&file,name, "rb");
	if (!file)
		;//((char*)"El archivo de entrada no se ha podido encontrar");
	//Get file length
	fseek(file, 0, SEEK_END);
	fileLen=ftell(file);
	fseek(file, 0, SEEK_SET);
...
RSA* getprivateRSAkey(void){
	...
	RSA* prikey;
	prikey = NULL;
 	if(0 != ( fopen_s(&fp,PRIV_KEY_PATH, "r")) ){
		prikey =PEM_read_RSAPrivateKey(fp,NULL,NULL,NULL);
          	if(prikey ==NULL)
  			;//((char*)"No pudimos cargar tu private RSA key");        
	}
...
RSA* getpublicRSAkey(char *host){
	...
	int  FOUND=0;
    FILE *wkh;
	fopen_s(&wkh,KH_FILE_PATH,"r");
        if(!wkh)
		;//((char*)"No he podido abrir el archivo de hosts\n");
	while ((fscanf(wkh, "%[^\n]", line)) != EOF )
   	{	
	        sscanf(line, "%[^',']",hostf);	
	...
	if(FOUND==0)
		;//((char*)"No he encontrado una llave publica para cifrar ");
	//if(strcmp(hostf,"-e")==0){	
	/*
	...
	*/
	fpss = NULL;
	if(NULL!= (fopen_s(&fpss,(const char*)cs,"r"))){
		pukey=PEM_read_RSAPublicKey(fpss,NULL,NULL,NULL);
		if(pukey==NULL)
			;//((char*)"No pudo cargarse la llave publica adecuada");
	}
	return pukey;
}
int addtowellknownhosts(char *host,char *ppath){
	...
	int i;
   	...
   	int bytes;
    	...
    	if (file == NULL) 
		;//((char*)"No puedo encontrar la llave especificada");
	...
	{	
                 printf("%02x", c[i]);
		 sprintf((char*)&cs[i*2], "%02x", (unsigned int)c[i]);
    		
	}
	printf (" %s\n", ppath);
    	fclose (file);
    	FILE *publica;
        fopen_s(&publica  ,KH_FILE_PATH,"a");
        if(!publica)
		;//((char*)"El archivo de salida no se ha podido crear");
        fprintf(publica,"%s,%s\n",host,ppath);
        fclose(file);
